...
public class PlayerPartAnimator extends LivingEntityPartAnimator<PlayerEntity, PlayerEntityModel<PlayerEntity>> {
    ...
    protected void buildRig(LocatorRig locatorRig) {
        ...
        locatorRig.addLocatorModelPart(locatorLeftLeg, locatorRightLeg, "left_leg", ModelTransform.pivot(1.9f, 12.0f, 0.0f));
        locatorRig.addLocatorModelPart(locatorRightLeg, locatorLeftLeg, "right_leg", ModelTransform.pivot(-1.9f, 12.0f, 0.0f));
        locatorRig.addLocatorModelPart(locatorLeftArm, locatorRightArm, "left_arm", ModelTransform.pivot(5.0f, 2.0f, 0.0f));
        locatorRig.addLocatorModelPart(locatorRightArm, locatorLeftArm, "right_arm", ModelTransform.pivot(-5.0f, 2.0f, 0.0f));
        locatorRig.addLocatorModelPart(locatorCloak, "cloak", ModelTransform.of(0, 0, 2, 0, MathHelper.PI, 0));
    ...
    public void tick(LivingEntity livingEntity, EntityAnimationData entityAnimationData) {
        ...
        boolean isCrouching = livingEntity.isInSneakingPose();
        ...
        float moveAngleX = -MathHelper.sin(livingEntity.prevBodyYaw * MathHelper.PI / 180);
        float moveAngleZ = MathHelper.cos(livingEntity.prevBodyYaw * MathHelper.PI / 180);
        float deltaMovementX = (float) (livingEntity.getX() - livingEntity.prevX) / entityAnimationData.getValue(ANIMATION_SPEED);
        float deltaMovementZ = (float) (livingEntity.getZ() - livingEntity.prevZ) / entityAnimationData.getValue(ANIMATION_SPEED);
        ...
        entityAnimationData.incrementInTicksFromCondition(CLIMBING_UP_WEIGHT, livingEntity.isClimbing() && entityAnimationData.getValue(DELTA_Y) >= 0, 8, 8);
        entityAnimationData.incrementInTicksFromCondition(CLIMBING_DOWN_WEIGHT, livingEntity.isClimbing() && entityAnimationData.getValue(DELTA_Y) < 0, 8, 8);

        entityAnimationData.setValue(VISUAL_SWIMMING_WEIGHT, livingEntity.getLeaningPitch(1));

        entityAnimationData.incrementInTicksFromCondition(IN_WATER_WEIGHT, livingEntity.isTouchingWater(), 8, 8);
        entityAnimationData.incrementInTicksFromCondition(UNDER_WATER_WEIGHT, livingEntity.isSubmergedInWater(), 8, 8);
        ...
        entityAnimationData.incrementInFramesOrResetFromCondition(IS_PASSENGER_TIMER, !livingEntity.hasVehicle(), 17);

        entityAnimationData.incrementInTicksFromCondition(FALL_FLYING_WEIGHT, livingEntity.getRoll() > 0, 12, 12);

        entityAnimationData.setValue(ATTACK_TIMER, livingEntity.handSwingProgress);
        if(entityAnimationData.getValue(ATTACK_TIMER) <= 0.001F){
            PlayerPartAnimator.AttackType attackType;
            if(livingEntity.preferredHand == Hand.MAIN_HAND){
        ...
        entityAnimationData.setValue(TICKS_SINCE_ATTACKING, livingEntity.handSwingProgress == 0 ? entityAnimationData.getValue(TICKS_SINCE_ATTACKING) + 1 : 0);
        ...
        entityAnimationData.incrementInTicksFromCondition(HOLD_NORMAL_MAIN_WEIGHT, getArmPose(livingEntity, Hand.MAIN_HAND) == BipedEntityModel.ArmPose.ITEM, 10, 10);
        entityAnimationData.incrementInTicksFromCondition(HOLD_NORMAL_OFF_WEIGHT, getArmPose(livingEntity, Hand.OFF_HAND) == BipedEntityModel.ArmPose.ITEM, 10, 10);
    ...
    private void addPoseLayerAttack(){
        ...
        boolean isAttacking = getDataValue(ATTACK_TIMER) != 0 || livingEntity.handSwingProgress != 0;
    ...
    private void addPoseLayerSleep(){
        ...
        float sleepIdleTimer = new TimerProcessor(this.livingEntity.age)
                .speedUp(1)
                .repeat(sleepMasterTimelineGroup)
                .getValue();
        float sleepStartTimer = getDataValue(SLEEP_TIMER);

        float sleepWeight = livingEntity.getPose() == EntityPose.SLEEPING ? 1 : 0;
    ...
    private void addPoseLayerMinecart(){
        if(livingEntity.hasVehicle() && livingEntity.getVehicle() instanceof AbstractMinecartEntity){
            ...
            float minecartIdleTimer = new TimerProcessor(this.livingEntity.age)
    ...
    private void addPoseLayerBoat(){
        ...
        if(livingEntity.hasVehicle() && livingEntity.getVehicle() instanceof BoatEntity){

            float boatIdleTimer = new TimerProcessor(this.livingEntity.age)
    ...
    private void addPoseLayerMount(){
        ...
        if(livingEntity.hasVehicle() && livingEntity.getVehicle() instanceof LivingEntity){
            ...
            float mountIdleTimer = new TimerProcessor(this.livingEntity.age)
    ...
    private void addPoseLayerCrawling(){
        ...
        float crawlWeight = Easing.CubicBezier.bezierInOutSine().ease(getDataValue(VISUAL_SWIMMING_WEIGHT) * 4 - 3)
                * (1 - getDataValueEasedQuad(IN_WATER_WEIGHT))
                * (1 - getDataValueEasedQuad(UNDER_WATER_WEIGHT));
        float crawlForwardWeight = MathHelper.lerp(getDataValueEasedQuad(DIRECTION_SHIFT), crawlWeight, 0);
        float crawlBackwardsWeight = MathHelper.lerp(getDataValueEasedQuad(DIRECTION_SHIFT), 0, crawlWeight);
    ...
    private void addPoseLayerSwimming(){
        ...
        float swimIdleForwardWeight = MathHelper.lerp(getDataValueEasedQuad(DIRECTION_SHIFT), swimIdleMoveWeight, 0);
        float swimIdleBackwardsWeight = MathHelper.lerp(getDataValueEasedQuad(DIRECTION_SHIFT), 0, swimIdleMoveWeight);

        float swimIdleUpForwardWeight = MathHelper.lerp(getDataValueEasedQuad(DIRECTION_SHIFT), swimUpMoveWeight, 0);
        float swimIdleUpBackwardsWeight = MathHelper.lerp(getDataValueEasedQuad(DIRECTION_SHIFT), 0, swimUpMoveWeight);

        float swimIdleSlowTimer = new TimerProcessor(this.livingEntity.age)
                .repeat(swimIdleTimelineGroup)
                .getValue();

        float swimIdleUpTimer = new TimerProcessor(this.livingEntity.age)
                .speedUp(1.5F)
                .repeat(swimIdleTimelineGroup)
                .getValue();
        ...
        float swimMasterTimer = (float) MathHelper.clamp((Math.toRadians(livingEntity.getPitch()) / MathHelper.PI) + 0.5F, 0, 1);
    ...
    private void addPoseLayerFallFlying(){
        ...
        float fallFlyingMasterTimer = (float) MathHelper.clamp((Math.toRadians(livingEntity.getPitch()) / MathHelper.PI) + 0.5F, 0, 1);
        ...
        this.locatorHead.rotateX -= MathHelper.HALF_PI * fallFlyingMasterWeight;
    ...
    private void addPoseLayerIdle(){
        ...
        float idleNormalTimer = new TimerProcessor(this.livingEntity.age)
        ...
        float idleNormalWeight = MathHelper.lerp(getDataValueEasedQuad(CROUCH_WEIGHT), idleWeight, 0);
        float idleCrouchWeight = MathHelper.lerp(getDataValueEasedQuad(CROUCH_WEIGHT), 0, idleWeight);
    ...
    protected void finalizeModelParts(ModelPart rootModelPart) {
        rootModelPart.getChild("left_pants").copyTransform(rootModelPart.getChild("left_leg"));
        rootModelPart.getChild("right_pants").copyTransform(rootModelPart.getChild("right_leg"));
        rootModelPart.getChild("left_sleeve").copyTransform(rootModelPart.getChild("left_arm"));
        rootModelPart.getChild("right_sleeve").copyTransform(rootModelPart.getChild("right_arm"));
        rootModelPart.getChild("jacket").copyTransform(rootModelPart.getChild("body"));
        rootModelPart.getChild("hat").copyTransform(rootModelPart.getChild("head"));
        rootModelPart.getChild("cloak").pitch *= -1F;
        // Removes the vanilla transformation done for the crouch pose
        if(this.entityModel.sneaking){
            for(ModelPart modelPart : rootModelPart.traverse().toList()){
                modelPart.pivotY -= 2;
            }
        }
    }

    private static BipedEntityModel.ArmPose getArmPose(LivingEntity livingEntity, Hand interactionHand) {
        ItemStack itemStack = livingEntity.getStackInHand(interactionHand);
        if (itemStack.isEmpty()) {
            return BipedEntityModel.ArmPose.EMPTY;
        }
        if (livingEntity.getActiveHand() == interactionHand && livingEntity.getItemUseTimeLeft() > 0) {
            UseAction useAnim = itemStack.getUseAction();
            ...
            if (useAnim == UseAction.CROSSBOW && interactionHand == livingEntity.getActiveHand()) {

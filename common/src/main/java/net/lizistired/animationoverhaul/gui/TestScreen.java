...
public class TestScreen extends Screen {
    ...
    public void render(@NotNull MatrixStack poseStack, int mouseX, int mouseY, float partialTicks) {
        ...
        poseStack.push();
        poseStack.translate(MathHelper.lerp(0.5, this.width / 3F, this.width), MathHelper.lerp(0.9, 48, this.height - 24), 0.0);
        poseStack.multiply(Vec3f.POSITIVE_X.getDegreesQuaternion(-10));
        poseStack.multiply(Vec3f.POSITIVE_Y.getDegreesQuaternion(-45));
        poseStack.multiply(Vec3f.POSITIVE_Y.getDegreesQuaternion((float) MathHelper.lerp(Math.sin(Util.getMeasuringTimeMs() / 1000F), 0, 20)));
        float scale = 50;
